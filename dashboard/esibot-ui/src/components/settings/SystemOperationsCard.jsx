...
export default function SystemOperationsCard() {
  ...
  useEffect(() => {
    ...
    onMessage("storageinfo", storageHandler);

    sendCommand("storageinfo");

    return () => {
      offMessage("sysinfo", sysHandler);
      offMessage("storageinfo", storageHandler);
    };
  }, []);

 const handlePowerOn = () => {
  sendCommand("bash -c 'source /opt/ros/humble/setup.bash && source ~/esibot_ws/install/setup.bash && ros2 launch rosbridge_server rosbridge_websocket_launch.xml & sleep 2 && ros2 launch esibot_bringup esibot_driver.launch.py &'");
};
